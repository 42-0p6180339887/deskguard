...
公开版本包含 **43 项离线测试**，覆盖输入过滤、照片存储、异常清理、托盘恢复和发布包隔离；已完成便携启动、自检、托盘与快捷键注册、辅助进程清理验证。测试使用模拟图像，**尚未进行真实摄像头、输入钩子、触屏及 Codex 共存的硬件实测**。通过离线测试不代表所有设备和输入场景可用。详见 [验证记录](docs/VALIDATION.zh-CN.md)；首次使用请按使用说明完成本机验收。

## 许可

项目原创源码与文档采用 [MIT License](LICENSE)。便携包中 Python、Tcl/Tk 及其他运行时组件保留各自的许可；见 [第三方声明](THIRD_PARTY_NOTICES.md)。
