...
桌宠按经过时间计算位置，预先缓存 16 个步态与 6 个转身帧；每次绘制只更新位置和已有图像。步态用预乘透明度插值，避免透明边缘变暗。独立窗口通过标准输入管道感知父程序退出。`DesktopPet.configure` 设置大小、速度和平稳动作；操作系统关闭界面动画时停止位移。控制面板预览使用独立的 6 秒计时器，不调用相机或输入监测入口。
